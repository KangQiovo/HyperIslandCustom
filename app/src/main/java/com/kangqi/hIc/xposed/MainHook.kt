...
class MainHook : IXposedHookLoadPackage {
    ...
    companion object {
        private const val MY_PACKAGE = "com.kangqi.hic"
    ...
    private fun hookSelf(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        // Report Xposed framework API version and detected framework to DeviceInfoHelper
        try {
            val apiVersion = XposedBridge.getXposedVersion()
            val helperClass = XposedHelpers.findClass(
                "com.kangqi.hIc.utils.DeviceInfoHelper", lpparam.classLoader
            )
            XposedHelpers.setStaticIntField(helperClass, "xposedApiVersion", apiVersion)
            XposedBridge.log("[HIC] Xposed API version: $apiVersion")

            // Detect which framework actually loaded us by checking for LSPatch-specific classes.
            // LSPatch patches the APK and injects its loader into the app's classloader,
            // so if the LSPatch loader class exists, we're running under LSPatch.
            // Otherwise, we're running under LSPosed (or another Zygisk-based framework).
            val frameworkName = try {
                Class.forName("org.lsposed.lspatch.loader.LSPApplication")
                "LSPatch"
            } catch (_: ClassNotFoundException) {
                "LSPosed"
            }
            XposedHelpers.setStaticObjectField(helperClass, "hookDetectedFramework", frameworkName)
            XposedBridge.log("[HIC] Detected framework: $frameworkName")
        } catch (e: Throwable) {
            XposedBridge.log("[HIC] Failed to report framework info: ${e.message}")
        }
    }
